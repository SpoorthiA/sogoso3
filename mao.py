...
def parallel_domain_executor(state: GraphState) -> Dict[str, Any]:
    ...
    print(f"STEP: Executing Domain Agents | Running {len(plan)} agents in parallel")
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in as_completed(future_to_agent):
            ...
            if error:
                print(f"INFO: {agent_name} failed ({elapsed:.2f}s): {error}")
                errors[agent_name] = error
            else:
                print(f"INFO: {agent_name} completed ({elapsed:.2f}s)")
    ...
    if client:
        try:
            ...
            print(f"INFO: Domain-Agents logging error: {str(e)}")
    
    print(f"INFO: Domain agents complete: {len(results)}/{len(plan)} in {total_time:.2f}s")
...
def should_evaluate(state: GraphState) -> str:
    ...
    if state.get("quality_evaluation_enabled"):
        return "evaluate"
    return "end"
...
def run_mao_with_observability(
    ...
    initial_state = {
        ...
        "quality_evaluation_enabled": enable_evaluation,
        "evaluation_scores": {}
    }
