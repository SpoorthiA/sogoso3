...
def preprocessing_agent(state: GraphState) -> Dict[str, Any]:
    ...
    """
    print("🔍 Preprocessing Agent: Extracting entities...")
...
def planning_agent(state: GraphState) -> Dict[str, Any]:
    ...
    """
    print("📋 Planning Agent: Creating execution plan...")
    ...
    with trace_agent(client, "Planning-Agent", {"entities": entities.model_dump() if entities else {}}, {"step": "agent_planning"}):
        ...
        try:
            ...
            # Print execution plan and subqueries in a single block
            num_steps = len(plan)
            plan_output = [f"{num_steps} step{'s' if num_steps != 1 else ''} planned"]
            
            # Map agent names to friendly names
            agent_display_names = {
                "product_agent": "Product Agent",
                "knowledge_agent": "Knowledge Agent",
                "promotion_agent": "Promotion Agent"
            }
            
            for i, agent_name in enumerate(plan, 1):
                display_name = agent_display_names.get(agent_name, agent_name)
                subquery = subqueries.get(agent_name, "")
                plan_output.append(f"{i}. {display_name}")
                if subquery:
                    plan_output.append(f'   "{subquery}"')
...
def product_agent(state: GraphState) -> Dict[str, Any]:
    ...
    """
    print("🏪 Product Agent: Searching products...")
    ...
    if agent_subquery:
        print(f"  Using subquery: {agent_subquery}")
    ...
    with trace_agent(client, "Product-Agent", {
        ...
        try:
            ...
            _print_retrieved_chunks(
                "product",
                products,
                lambda p: f"{p['name']} - ${p['price']} ({p['brand']}) - {p['description'][:80]}..."
            )
...
def knowledge_agent(state: GraphState) -> Dict[str, Any]:
    ...
    """
    print("📚 Knowledge Agent: Searching policies...")
    ...
    with trace_agent(client, "Knowledge-Agent", {
        ...
        try:
            ...
            _print_retrieved_chunks(
                "knowledge",
                policies,
                lambda p: f"{p['title']} - {p['content'][:80]}..."
            )
...
def promotion_agent(state: GraphState) -> Dict[str, Any]:
    ...
    """
    print("🎁 Promotion Agent: Searching promotions...")
    ...
    with trace_agent(client, "Promotion-Agent", {
        ...
        try:
            ...
            _print_retrieved_chunks(
                "promotion",
                promotions,
                lambda p: f"{p['name']} ({p['type']}) - {p['content'][:80]}..."
            )
